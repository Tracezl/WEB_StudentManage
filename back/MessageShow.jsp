				 ...
				 <center>  
						 <h1>留言信息</h1><hr>
							  ...
							  int id=0;
							   %>
							   <table border="1">
							   <tr id="head"><td>ID</td><td>内容</td><td>删除</td>
							   </tr>   
							   <%
							   int pageNow=Integer.parseInt((String )request.getAttribute("pageNow"));//当前页码
							     for(int i=0;i<al.size();i++){
							     Message ub=(Message)al.get(i);
							   %>    
							   <tr >
							   <td ><%=ub.getId()%></td>
							   <td><%=ub.getContent()%></td>
							   <td><a onclick="return abc();" href="MessageCL?flag=Del&Id=<%=ub.getId()%>">删除留言</a></td>
							   </tr> 
							   <% }%> 
							  </table><br>
							  <h3>
							  <%
							  if(pageNow!=1){
							     out.println("<a href=MessageCL?flag=seek&pageNow="+(pageNow-1)+" >上一页</a>"); 
							  }
							  String s_pageCount=(String)request.getAttribute("pageCount");
							  int pageCount=Integer.parseInt(s_pageCount);
							  
							  for(int i=1;i<=pageCount;i++){
							   out.println("<a href=MessageCL?flag=seek&pageNow="+i+" >["+i+"]</a>");
							  }
							  if(pageNow!=pageCount&&pageCount!=0){
							     out.println("<a href=MessageCL?flag=seek&pageNow="+(pageNow+1)+" >下一页</a>"); 
							  }
							  %><br><br><br><br>
				 </center>
			</div>
